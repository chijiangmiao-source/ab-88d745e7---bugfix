...
function verifyChain(input) {
  ...
  for (const { model: m } of hops) {
    ...
    finalCap = Math.min(finalCap, m.maxSamples);
  }
  for (let i = 0; i < hops.length; i++) {
    ...
    hopEvidence.push({
      ...
      tightened: { nbf: m.nbf, exp: m.exp, aud: [...m.aud], maxSamples: m.maxSamples },
    });
  }
  ...
  return {
    ...
    evidence: {
      ...
      hops: hopEvidence,
      finalConstraints: { nbf: finalNbf, exp: finalExp, aud: finalAud, maxSamples: finalCap },
      verdict: {
        allow: true,
        buoy: last.buoy,
