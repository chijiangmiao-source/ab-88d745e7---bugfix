...
> 主体允许在链中回环出现（如 root→A→B→A→C）。收紧比较一律以**紧邻的上一跳**
> 为准（上一跳 `sub` 经验签即本跳 `iss`，故它是该主体签发时持有的最新授权），
> 不复用同一主体早期更宽的授权；末端命令则始终受**每一跳**（含回环中的收紧跳）
> 约束。因此 A 在被 B 收紧后再次向 C 下放更宽范围，会在该下放跳以 `NOT_TIGHTENED`
> 被拒绝，并定位首个被放宽的字段——已签发、已验证的后续命令无法掩盖中途收紧。
